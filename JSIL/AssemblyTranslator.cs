...
namespace JSIL {
    public class AssemblyTranslator : IDisposable {
        ...
        protected void TranslateInterface (DecompilerContext context, JavascriptFormatter output, TypeDefinition iface) {
			output.Identifier("JSIL.MakeInterface", (EscapingMode?)null);
            output.LPar();
            output.NewLine();
            ...
            foreach (var methodGroup in iface.Methods.GroupBy(md => md.Name)) {
                foreach (var m in methodGroup) {
					...
					output.Identifier("$", (EscapingMode?)null);
                    output.Dot();
					output.Identifier("Method", (EscapingMode?) null);
                    output.LPar();
            ...
            foreach (var p in iface.Properties) {
				...
				output.Identifier("$", (EscapingMode?)null);
                output.Dot();
				output.Identifier("Property", (EscapingMode?) null);
                output.LPar();
        ...
        protected void TranslateEnum (DecompilerContext context, JavascriptFormatter output, TypeDefinition enm) {
			...
			output.Identifier("JSIL.MakeEnum", (EscapingMode?)null);
            output.LPar();
            output.NewLine();
        ...
        protected void TranslateDelegate (DecompilerContext context, JavascriptFormatter output, TypeDefinition del, TypeInfo typeInfo) {
			output.Identifier("JSIL.MakeDelegate",(EscapingMode?) null);
            output.LPar();
        ...
        protected void DeclareType (
            ...
            try {
                ...
                if (typeInfo.IsExternal) {
					output.Identifier("JSIL.MakeExternalType",(EscapingMode?) null);
                    output.LPar();
				...
				Action<JavascriptFormatter> dollar = (o) => o.Identifier("$", (EscapingMode?)null);
                ...
                if (makingSkeletons) {
					output.Identifier("JSIL.ImplementExternals", (EscapingMode?)null);
                    output.LPar();

                    output.Value(typeInfo.FullName);

                } else if (isStatic) {
					output.Identifier("JSIL.MakeStaticClass",(EscapingMode?) null);
                    output.LPar();
                    ...
                    if (typedef.IsValueType)
						output.Identifier("JSIL.MakeStruct", (EscapingMode?)null);
                    else
						output.Identifier("JSIL.MakeClass", (EscapingMode?)null);
        ...
        protected void TranslatePrimitiveDefinition (
            ...
            var setValue = (Action<string, bool>)((name, value) => {
                dollar(output);
                output.Dot();
				output.Identifier("SetValue", (EscapingMode?)null);
                output.LPar();
                output.Value(name);
                output.Comma();
                output.Value(value);
        ...
        protected void TranslateTypeDefinition (
            ...
            if (!makingSkeletons && (interfaces.Length > 0)) {
                ...
                dollar(output);
                output.Dot();
				output.Identifier("ImplementInterfaces", (EscapingMode?)null);
                output.LPar();
        ...
        protected void DefineMethod (
            ...
            try {
                ...
                if (isExternal && !Configuration.GenerateSkeletonsForStubbedAssemblies.GetValueOrDefault(false))
					output.Identifier("ExternalMethod", (EscapingMode?)null);
                else
					output.Identifier("Method", (EscapingMode?)null);
                output.LPar();
                ...
                if (!isExternal) {
                    ...
                    if (function != null) {
						...
						output.Identifier("JSIL.UntranslatableFunction", (EscapingMode?)null);
                        output.LPar();
        ...
        protected void TranslateProperty (
            ...
            if (propertyInfo.IsExternal)
				output.Identifier("ExternalProperty", (EscapingMode?)null);
            else if (property.DeclaringType.HasGenericParameters && isStatic)
				output.Identifier("GenericProperty", (EscapingMode?)null);
            else
				output.Identifier("Property", (EscapingMode?)null);
